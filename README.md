...
- **9. В меню Visual Studio Code "Terminal" - "Run Task..." доступно три задачи: "Build", "Program", "Clean" - согласно настроек в п. 5.4 для соответственно сборки проекта, загрузки прошивки в микроконтроллер и очистки проекта.** 
...
- **12. Настройка порта SWO микроконтроллера STM32 для вывода отладочных сообщений в случае, если по вышеприведенной методике порт не заработал.**
    ...
    - 12.2. Создать проект в STM32CubeIDE для своей модели микроконтроллера STM32, активировать порт SWO и скомпилировать проект (см. документацию STMicroelectronics). Далее необходимо будет скопировать некоторые файлы из STM32CubeIDE в проект Visual Studio Code.
